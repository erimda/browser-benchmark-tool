...
RSpec.describe BrowserBenchmarkTool::ReportGenerator do
  let(:config) do
    BrowserBenchmarkTool::Config.new.tap do |c|
      ...
      c.thresholds = { latency_threshold_x: 2.0, cpu_threshold: 0.9, mem_threshold: 0.8, error_rate_threshold: 0.01 }
      c.output = { dir: './artifacts', generate_charts: true }
    end
  end
  ...
  describe '#get_maximum_sustainable_concurrency' do
    context 'when degradation is detected' do
      before do
        allow(degradation_engine).to receive_messages(degradation_detected?: true,
                                                      get_maximum_sustainable_concurrency: 1)
      end
