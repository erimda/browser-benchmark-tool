...
RSpec.describe BrowserBenchmarkTool::Benchmark do
  let(:config) do
    BrowserBenchmarkTool::Config.new.tap do |c|
      c.workload = {
        ...
        engine: 'chromium',
        headless: true,
        urls: ['https://httpbin.org/delay/0.1'],
        per_browser_repetitions: 2,
        min_level_seconds: 5
      }
      c.ramp = {
  ...
  describe '#run' do
    ...
    it 'handles degradation detection correctly' do
      ...
      allow(benchmark.degradation_engine).to receive(:degradation_detected?).and_return(true)
      allow(benchmark.degradation_engine).to receive(:stop_reason).and_return('Latency threshold exceeded')
  ...
  describe 'error handling' do
    it 'handles network errors gracefully' do
      ...
      config.workload[:urls] = ['https://invalid-domain-that-does-not-exist-12345.com']
    ...
    it 'handles single URL' do
      config.workload[:urls] = ['https://httpbin.org/delay/0.1']
  ...
  describe 'configuration variations' do
    it 'works with different ramp strategies' do
      config.ramp = { strategy: 'linear', levels: [1, 2, 3] }
      
      expect { benchmark.run }.not_to raise_error
    end

    it 'works with custom ramp levels' do
      config.ramp = { strategy: 'custom', levels: [1, 3, 5] }
  ...
  describe 'performance characteristics' do
    it 'completes within reasonable time for small test' do
      ...
      expect(duration).to be < 30.0 # Should complete in under 30 seconds
    end
