...
RSpec.describe BrowserBenchmarkTool::CLI do
  ...
  describe '#benchmark' do
    it 'runs benchmark with default configuration' do
      ...
      config.workload = { mode: 'playwright', engine: 'chromium', headless: true, urls: ['https://httpbin.org/delay/0.1'], per_browser_repetitions: 1, min_level_seconds: 5 }
      ...
      config.safety = { robots_txt_respect: false, external_rate_limit_rps: 10, max_concurrent_requests: 20, request_timeout_seconds: 10, max_total_requests: 50 }
    ...
    it 'runs benchmark with CLI options' do
      cli = described_class.new
      # Use Thor's invoke method to set options
      expect { cli.invoke(:benchmark, [], { urls: ['https://httpbin.org/delay/0.1'], ramp: 'exp:1', reps_per_level: 1, level_min_seconds: 5, out_dir: './test_artifacts' }) }.not_to raise_error
    ...
    it 'handles missing configuration file gracefully' do
      ...
      cli = described_class.new
      expect { cli.invoke(:benchmark, [], { urls: ['https://httpbin.org/delay/0.1'], ramp: 'exp:1' }) }.not_to raise_error
  ...
  describe '#load_config' do
    it 'loads configuration from YAML file' do
      config = BrowserBenchmarkTool::Config.new
      config.workload = { mode: 'playwright', engine: 'chromium', headless: true, urls: ['https://example.com'], per_browser_repetitions: 3, min_level_seconds: 30 }
      config.ramp = { strategy: 'exponential', levels: [1, 2, 4] }
      config.thresholds = { latency_threshold_x: 2.0, cpu_threshold: 0.8, mem_threshold: 0.8, error_rate_threshold: 0.05 }
      config.output = { dir: './artifacts', max_runtime_minutes: 20, generate_charts: true }
      config.safety = { robots_txt_respect: true, external_rate_limit_rps: 2, max_concurrent_requests: 10, request_timeout_seconds: 30, max_total_requests: 100 }
      ...
      expect(loaded_config.workload[:urls]).to eq(['https://example.com'])
      expect(loaded_config.ramp[:levels]).to eq([1, 2, 4])
    ...
    it 'merges CLI options with YAML configuration' do
      ...
      cli.instance_variable_set(:@options, { config: 'benchmark.yml', urls: ['https://httpbin.org/delay/0.1'], ramp: 'exp:1,2' })
      
      loaded_config = cli.send(:load_config)
      
      expect(loaded_config.workload[:urls]).to eq(['https://httpbin.org/delay/0.1'])
