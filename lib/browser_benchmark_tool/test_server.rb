...
module BrowserBenchmarkTool
  class TestServer
    ...
    def generate_response(method, path)
      ...
      when '/slow'
        sleep(rand(0.2..0.6))
        generate_html_response('Slow Response', 'Slow Response', 'This endpoint has a controlled delay for testing latency.')
      when '/flaky'
        if rand < 0.2
          generate_error_response(500, 'Internal Server Error', 'Simulated error for testing error handling.')
        else
          generate_html_response('Flaky Response', 'Flaky Response', 'This endpoint occasionally fails for testing error rates.')
        end
      when '/heavy'
        large_content = Array.new(1000) { |i| "<p>Heavy content line #{i + 1}</p>" }.join("\n")
        generate_html_response('Heavy Load', 'Heavy Load', 'This endpoint serves large content for testing memory usage.', large_content)
